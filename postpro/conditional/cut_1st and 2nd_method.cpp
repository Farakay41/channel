...
Matrix2 cut_velocity(std::vector <double> large, std::vector <double> small, std::vector <int> positions, int n, double dt)
...
int main()
{
	...
	cut_us_P = cut_velocity(ul, us, array_pos, num_of_cuts, dt_);

	//call the cut_velocity function to calculate the averaged velocity signals (negative to positive)
	cut_us_N = cut_velocity(ul, us, array_neg, num_of_cuts, dt_);
	...
	cut_us_Pos = average_cut_velocity(cut_us_P[0], array_pos, num_of_cuts);
	cut_us_Neg = average_cut_velocity(cut_us_N[0], array_neg, num_of_cuts);
	...
	{
		file_P.write((char*)(&cut_us_Pos[i]), sizeof(double));
	}
	...
	{
		file_N.write((char*)(&cut_us_Neg[i]), sizeof(double));
	}


	std::fstream time_file_P;
	time_file_P.open("time_pos_neg.out", std::ios::out | std::ios::binary);
	for (int i = 0; i < cut_us_P[1].size(); i++)
	{
		time_file_P.write((char*)(&cut_us_P[1][i]), sizeof(double));
	}

	std::fstream time_file_N;
	time_file_N.open("time_neg_pos.out", std::ios::out | std::ios::binary);
	for (int i = 0; i < cut_us_N[1].size(); i++)
	{
		time_file_N.write((char*)(&cut_us_N[1][i]), sizeof(double));
	}
	...
	*/
	...
	if (out_pos)
	{
		out_pos.write(reinterpret_cast<char*>(cut_us_P[0].data()), static_cast<std::streamsize>(cut_us_P[0].size()) * sizeof(double));
		out_pos.close();
	...
	if (out_neg)
	{
		out_neg.write(reinterpret_cast<char*>(cut_us_N[0].data()), static_cast<std::streamsize>(cut_us_N[0].size()) * sizeof(double));
		out_neg.close();
